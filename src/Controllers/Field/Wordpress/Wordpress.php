...
namespace crewstyle\TeaThemeOptions\Controllers\Field\Wordpress;
 ...
 * @subpackage Controllers\Field\Wordpress
    ...
    protected function getWPPosttypes($options = array())
    {
        ...
        if (!empty($types_obj)) {
            foreach ($types_obj as $typ) {
                //Get the the name
                $contents[0][$typ->name] = $typ->labels->name;
            }
        }
    ...
    protected function getWPTerms($options = array())
    {
        ...
        if (!empty($taxs_obj)) {
            foreach ($taxs_obj as $tax) {
                //Get the id and the name
                $contents[0][$tax->term_id] = $tax->name;
            }
        }
